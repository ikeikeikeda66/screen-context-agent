...
| 9 | Code signing | **No Apple Developer ID.** Use a fixed self-signed identity so Screen Recording permission survives updates. Verified in #7: the grant survives a rebuild; an unnotarized app from a quarantined download is blocked by Gatekeeper, so users build the app on their own Mac. |
